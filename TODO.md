...
- **[fetcher]** `claimweb.fetchers.naic_schedule_d`: NAIC Schedule D security-by-security (per project plan §10.3) — spawn `data-source-investigator` first
...
- **[fetcher]** 2026-05-15 — Implemented `claimweb.fetchers.naic_schedule_s`: `NaicScheduleSFetcher` for NAIC Schedule S reinsurance cessions. Reads CSV from cache; emits A6 arcs (source=reinsurer, target=cedent); amounts in $000→millions; DIRECT_MEASURED quality; annual/Q4 cadence. Offshore-domicile detection (BM, KY, IE, etc.) in validate(). PE-affiliated cedent tracking (Athene 68039, Global Atlantic 97071, F&G 63177). 99 tests (3 property-based); 976 total pass; gate green. See CHANGELOG 2026-05-15 "fetcher: NAIC Schedule S reinsurance cessions". Commit: e5bd591.
